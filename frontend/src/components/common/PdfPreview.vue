...
function loadPdfFromUrl(url: string) {
  ...
  // 先尝试从URL中直接提取标题信息
  try {
    console.log('Attempting to set document title early from URL...')
    const pathSegments = url.split('/')
    const fileNameWithParams = pathSegments[pathSegments.length - 1]
    let fileName = fileNameWithParams.split('?')[0] // 移除URL参数
    ...
    try {
      ...
      console.error('Failed to decode filename from URL', e)
    }

    // 检查是工艺PDF还是图纸PDF
    if (url.includes('-工艺') || url.includes('-process') ||
      fileName.includes('-工艺') || fileName.includes('-process')) {
      console.log('Detected process PDF from URL')

      // 从URL中查找可能包含代码和公司信息的段
      for (let i = pathSegments.length - 1; i >= 0; i--) {
        const segment = pathSegments[i]
        if (segment.includes('-工艺') || segment.includes('-process')) {
          console.log('Found process segment:', segment)

          // 提取代码和公司信息
          let cleanSegment = segment.split('?')[0] // 移除URL参数

          if (cleanSegment.includes('-工艺')) {
            const parts = cleanSegment.split('-工艺')[0].split('-')
            if (parts.length >= 2) {
              const code = parts[0]
              const company = parts.slice(1).join('-')
              document.title = `${code}-${company}-工艺.pdf`
              console.log('Set document title early to:', document.title)
            }
          } else if (cleanSegment.includes('-process')) {
            const parts = cleanSegment.split('-process')[0].split('-')
            if (parts.length >= 2) {
              const code = parts[0]
              const company = parts.slice(1).join('-')
              document.title = `${code}-${company}-工艺.pdf`
    ...
    } else if (url.includes('-图纸') || fileName.includes('-图纸')) {
      console.log('Detected drawing PDF from URL')

      // 从URL中查找可能包含代码和公司信息的段
      for (let i = pathSegments.length - 1; i >= 0; i--) {
        const segment = pathSegments[i]
        if (segment.includes('-图纸')) {
          console.log('Found drawing segment:', segment)

          // 提取代码和公司信息
          let cleanSegment = segment.split('?')[0] // 移除URL参数

          const parts = cleanSegment.split('-图纸')[0].split('-')
          if (parts.length >= 2) {
            const code = parts[0]
            const company = parts.slice(1).join('-')
            document.title = `${code}-${company}-图纸.pdf`
    ...
    console.error('Error setting early document title:', e)
  }
